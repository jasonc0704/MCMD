...
"""
Created on Mon Feb  1 2022

@author: Zhenzi Yu, Chao-Wen Chang
"""
...
def GenAdsorbateData(framework = None, adsorbate = None, cyclenumber = None):
    ...
    ### record offset
        if desiredOrder[i]!=keyOutput[i]:
            return "framwork.lmps keyword order not correct, please change the order"
...
def updateRestart(NUMofad, a, f_lmps=None):

    restart_file = os.listdir('Restart/System_0/')[0]

    with open(f_lmps, 'r') as f:
        LINES = f.readlines()
        ...
        pos = np.zeros(shape=(end_index-start_index+1,6))
        j=0
        for i, line in enumerate(LINES):
            if i >= start_index and i <= end_index:
                pos[j,0] = int(line.split()[0])
                pos[j,3] = float(line.split()[4])
                pos[j,4] = float(line.split()[5])
                pos[j,5] = float(line.split()[6])
                j=j+1
    ...
    with open('Restart/System_0/'+restart_file,'r') as f1:
      
        LINES1 = f1.readlines()
        for i, line in enumerate(LINES1):
            if 'unit-cell-vector-a' in line:
                param = line.split()[1]
        
    with open('Restart/System_0/'+restart_file,'r') as f1:
        filedata = f1.read()
        
    filedata = filedata.replace(param, str(a))
    
    with open('Restart/System_0/'+restart_file, 'w') as f1:
        f1.write(filedata)
                
    with open('Restart/System_0/'+restart_file,'r') as f1:
        LINES1 = f1.readlines()
        for i, line in enumerate(LINES1):
            if 'Reactions: 0' in line:
                trun=i
                
        head = list(islice(LINES1, trun+1))
        
    with open("Restart/System_0/out.txt", "w") as f2:
            
        for item in head:
            f2.write(item)
        f2.write("\nComponent: 0     Adsorbate    "+str(NUMofad)+" molecules of CO2\n")
        f2.write("------------------------------------------------------------------------\n")
        for k in range(pos.shape[0]):
            f2.write("Adsorbate-atom-position: "+str(int(pos[k,0]))+"  "+str(int(pos[k,1]))+"  "+str(pos[k,2])+" "+str(pos[k,3])+" "+str(pos[k,4])+"\n")
        f2.close()
        f1.close()   
...
def write_raspa_input(temp=None, pressure=None, cyclenumber=None, cycles=None,framework=None, adsorbate=None, rosenbluth=None,HeliumVoidFraction=None,startNUM=None):
    ...
    f.write('SimulationType                MonteCarlo\n')
    if cyclenumber ==0:
        f.write('NumberOfCycles                ' + str(cycles*10)+'\n')
        f.write('NumberOfInitializationCycles  ' + str(cycles*10)+'\n')
    else:
    ...
    f.write('CreateNumberOfMolecules    0\n')
    print("write RASPA input file complete!")

    f.close()
    return

def write_lammps_input(temp=None, pressure=None, seed=None, md_time=None, cyclenumber=None,framework=None,adsorbate_Types=None,f_Types=None,elementArray=None):
    if temp is None:
        print('you must specify a temperature')
        return
    if pressure is None:
...
################################# pre-run setting #####################################################
            ...
            velocity	all create """+str(temp)+" "+str(seed)+""" rot yes mom yes dist gaussian\n\n
############################### NVT ###########################################################        
            reset_timestep 0
...
############################### NPT ###########################################################
            ...
            reset_timestep 0
            fix NPT_ENSEMBLE all npt temp """+str(temp)+' '+str(temp)+' 100 iso '+str(pressure)+' '+str(pressure)+' 1000'+"""
            fix     COM fram recenter INIT INIT INIT
            timestep 1.0
            run              """+str(md_time)+"""
            unfix NPT_ENSEMBLE
...
############################### NVT ###########################################################
    ...
    print("write lammps input file complete!")	
    f.close()
    return

def run_raspa(cyclenumber):
    global RASPA_EXEC
...
The folloing part is writen by Anstine without further testing by Chao-Wen
"""
...
def updatecif(f_lmps=None, f_cif=None, num_framework_atoms=None, cyclenumber=None):
    ...
    for line in f:
        ...
        if len(row) > 3 and row[2] == 'xlo':
            ...
            zhi = float(row[1])
            dz = zhi - zlo
            cellInfo,transM = _cellParameterInput2_(np.array([[xlo,xhi,0],[ylo,yhi,0],[zlo,zhi,0]],dtype=float))
        elif len(row) > 0 and row[0] == 'Atoms':
                start_parsing = True
    ...
    for line in f:
        ...
        if row[0] == '_cell_length_a':
            ...
            fl.write(row[0]+'\t'+row[1]+'\t'+str(atoms[count][0])+'\t'+str(atoms[count][1])+'\t'+str(atoms[count][2])+'\t'+row[5]+'\t'+str(atoms[count][3])+'\n')
            count += 1
            continue
        fl.write(line)
    f.close()
...
def updateFrameworkData(cyclenumber,framework):
    ...
    os.system('tail -n +10 fig/final_framework_1000000 > output_pos')
